...
GitHub Pages address (or a bit.ly mask in front of it — bit.ly's free tier now inserts an ad
interstitial before redirecting, on top of not showing click stats) visible to readers; the
Vercel URL above doesn't reveal the GitHub handle and redirects instantly, so it's used
directly with no extra masking layer — **and** the blog's own e-book launch/promo post (currently "드디어 첫 전자책을
...
읽어보기](...)`) is enough to make it stand out from the main content — separate the two link
lines with a blank line between them (marked doesn't turn a single `\n` into a `<br>`, so
without one they'd render glued onto the same line); don't wrap it in a
bordered/background box like a banner ad.

## Click tracking for e-book CTA links: the `/go/` redirector (GA4), served from Vercel
...
redirect page — source lives in this repo (`public/go/index.html`) and GitHub Pages still
serves it too, but **the URL actually used in published posts is the Vercel deployment**,
`https://blog-eta-blond-71.vercel.app/go/` (a Vercel project connected to this same repo,
auto-deploying on push to `master` same as GitHub Pages) — its domain doesn't reveal the
`turbofv-pixel` GitHub handle the way `turbofv-pixel.github.io` does, and unlike a bit.ly link
in front of it, it redirects instantly with no ad interstitial (bit.ly's free tier now shows
one, and doesn't expose click stats either — don't reintroduce a bit.ly layer here). Its GA4
property (measurement ID `G-FKW051Z13L`) logs an `outbound_click` event (with `link_label` and
`destination_url` params) before forwarding the visitor on to the real destination. URL shape:
`https://blog-eta-blond-71.vercel.app/go/?to=<url-encoded destination>&label=<short-label>`.
